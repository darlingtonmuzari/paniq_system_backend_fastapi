#!/usr/bin/env python3
"""
Insert sample emergency provider data into the database (simplified for existing schema)
"""
import asyncio
import sys
import os
...
async def insert_sample_data():
    ...
    try:
        ...
        async with async_session() as session:
            ...
            # Sample emergency providers data (only using columns that exist in the table)
            sample_providers = [
                {
                    'id': str(uuid4()),
                    'firm_id': firm_id,
            ...
            for provider_data in sample_providers:
                ...
                # Insert using raw SQL to avoid model mismatch
                await session.execute(text("""
                ...
                inserted_count += 1
                print(f"✅ Added: {provider_data['name']} ({provider_data['provider_type']})")
