#!/usr/bin/env python3
"""
Create sample panic requests for Paniq Security Solutions - Simple version
"""
...
async def create_sample_panic_requests():
    ...
    async with AsyncSessionLocal() as db:
        try:
            # First, check if we have existing user groups we can use
            ...
            if not existing_groups:
                print("No existing user groups found. Creating minimal test users...")
                
                # Create one test user and group
                user_id = str(uuid4())
                group_id = str(uuid4())
                ...
                # Use this group for the first request
            ...
            for i, request_data in enumerate(sample_requests):
                # Create panic request using the existing group (or newly created one)
                ...
                """), {
                    "id": request_id,
                    "group_id": group_to_use[0],  # Use the group ID
